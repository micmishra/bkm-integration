...
namespace BKM.Integration.Application.Features.Email.Services;
    ...
    public async Task<string> TestConnectionAsync(string toAddress, CancellationToken ct = default)
    {
        ...
        try
        {
            await sender.SendAsync(config, toAddress,
                ...
                body:    $"SMTP connection test from BKM.Integration at {DateTime.UtcNow:O}. If you received this, the configuration is working.",
                isHtml:  false, ct);
            sw.Stop();
