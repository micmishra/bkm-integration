...
using BKM.Integration.Application;
using BKM.Integration.Domain.Features.Auth.Entities;
using BKM.Integration.Infrastructure;
using BKM.Integration.Infrastructure.Persistence;
using BKM.Integration.Infrastructure.Features.AppLog.Logging;
using BKM.Integration.Api.Features.ApiResponse.Filters;
using BKM.Integration.Api.Features.ApiResponse.Middleware;
using Serilog;
...
try
{
    ...
    {
        c.SwaggerDoc("v1", new() { Title = "BKM Integration API", Version = "v1" });
...
{
    Log.Fatal(ex, "BKM.Integration failed to start.");
}
finally
{
    Log.CloseAndFlush();
